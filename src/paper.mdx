...
<Figure>
  <Comparison slot="figure" client:idle >
    ...
    <Picture slot="itemTwo" src={dogsTrue} alt="Photo of two dogs running side-by-side in shallow water" />
  </Comparison>
...
Carousels are useful for situations in which you'd like to display a collection of visuals without taking up much space and you're okay with the reader not seeing every visual. I created my own carousel component for this template because I wasn't satisfied with the existing carousel libraries; please [file an issue](https://github.com/RomanHauksson/academic-project-astro-template/issues/new) if you run into any bugs!

<Carousel>
  <CarouselSlide>
    <Picture src={dogsTrue} alt="Photo of two dogs running side-by-side in shallow water" />
  </CarouselSlide>
  <CarouselSlide>
    <Picture src={dogsDiffc} alt="Photo of two dogs running side-by-side in shallow water, lossily compressed using the DiffC algorithm" />
  </CarouselSlide>
  <CarouselSlide>
    You can put any element in each carousel slide, not just pictures. Since this text isn't part of a component, it will be _interpreted_ as **Markdown**.
  </CarouselSlide>
  <CarouselSlide>
    <Picture src={dogsTrue} alt="Photo of two dogs running side-by-side in shallow water" />
  </CarouselSlide>
