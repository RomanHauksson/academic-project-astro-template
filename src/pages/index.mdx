...
# The color theme of the page. Defaults to "device" (the preference set in the user's brower or operating system).
# Setting this to "light" or "dark" will override the user's preference. This is useful if your figures only look good in one theme.
theme: device
...
# After you publish your page, you can double check that these previews look right using [this tool](https://linkpreview.xyz/)
...
import RenderPDF from "../components/RenderPDF.astro"
...
If you stored your figures as PDFs, copy them into `./src/assets/` and use the `RenderPDF` component to convert them into web-friendly images automatically.

<Figure>
  <RenderPDF slot="figure" filename="allstate-sparsity.pdf" altText="Impact of the sparsity-aware algorithm in XGBoost on the Allstate-10K dataset." />
...
Use the `ImageComparison` component to compare two images with an interactive slider.

<Figure>
  <ImageComparison
    slot="figure" client:load
    imageOne={dogsDiffc} imageTwo={dogsTrue}
    altTextOne="Photo of two dogs running side-by-side in shallow water, lossily compressed using the DiffC algorithm"
    altTextTwo="Original photo of two dogs running side-by-side in shallow water"
  />
...
<TwoColumns>
  ...
  <Figure slot="right">
    <ThreeDimensional slot="figure" filename="BoxVertexColors.glb" client:load />
